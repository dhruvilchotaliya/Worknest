	...
	const columns: TableColumnDef<DashboardProject>[] = [
			...
			renderCell: (row) => (
				<div className="flex items-center gap-3">
					...
					<div className="flex flex-col">
						<Typography component="body2" testId="project-list-name" className="font-semibold text-slate-900">
							{row.name}
						</Typography>
						{row.code && (
							<Typography component="caption" testId="project-list-code" className="text-slate-400 font-mono text-[10px]">
								{row.code}
							</Typography>
						)}
					</div>
		...
		},
		{
			...
			renderCell: (row) => {
				...
				if (row.status === "Active") color = "bg-emerald-50 text-emerald-700 border-emerald-100";
				if (row.status === "OnHold") color = "bg-amber-50 text-amber-700 border-amber-100";
				if (row.status === "Completed") color = "bg-violet-50 text-violet-700 border-violet-100";
				if (row.status === "Cancelled") color = "bg-rose-50 text-rose-700 border-rose-100";
		...
		},
		{
			...
			width: 140,
			renderCell: (row) => (
				<Typography component="body2" testId="project-list-due-date" className="text-slate-500 font-sans text-xs">
					{row.dueDate || "-"}
				</Typography>
			),
		},
			...
			renderCell: (row) => (
				<Box className="flex items-center" >
					{row.members && row.members.length > 0 ? (
						<AvatarGroup max={4} sx={{ "& .MuiAvatar-root": { width: 24, height: 24, fontSize: "0.65rem", fontWeight: 700, border: "2px solid white" } }}>
							{row.members.map((m) => {
						...
						<Typography component="caption" testId="project-list-no-members" className="text-slate-400 font-sans">
							None
						</Typography>
					)}
				</Box>
